...
- **`/apple-design`** is the reference for interface *feel*, and the only design-craft skill in the set: springs instead of fixed durations, 1:1 pointer tracking, motion that starts from the live on-screen value so a gesture can grab and reverse it mid-flight, momentum projected forward at release, translucent materials, and size-specific tracking and leading. Model-invoked, so the agent reaches for it while building gesture-driven UI; reach for it by hand when a UI works but feels dead. It pairs with **`/prototype`**, since "does this feel right" is the question static design can't answer. Nothing to do with **`/hyperframes`**: that renders a deterministic timeline to frames, where interruptibility has no meaning. Vendored from [emilkowalski/skills](https://github.com/emilkowalski/skills), so expect a different voice.
