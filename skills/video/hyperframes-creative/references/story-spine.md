...
Applies to the narrated, story-driven creation workflows — `/product-launch-video`, and `/general-video` when the piece tells a story. It does **not** apply to `/motion-graphics` (no narration — motion is the message). Do not force these rules onto an exempt workflow.
