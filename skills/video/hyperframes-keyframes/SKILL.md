...
> ## ⚠ Contract status: craft is current, code is not
>
> This stack's output framework is **Remotion + React** (`/hyperframes-core`). The
> **motion craft here still applies**: timing, easing choices, the shape of a move,
> the named rules and blueprints. The **code does not**: examples and adapters are
> written against the retired HTML + paused-GSAP-timeline runtime.
>
> Translating as you read:
>
> | Here                                            | In Remotion                                                       |
> | ----------------------------------------------- | ----------------------------------------------------------------- |
> | `gsap.fromTo(el, {x:-40}, {x:0, duration:0.5})` | `interpolate(frame, [0, SEC(0.5)], [-40, 0], {…"clamp"})`          |
> | `ease: "back.out(1.7)"`                         | `spring({frame, fps, config:{damping}})`, retuned by eye           |
> | a paused timeline at `window.__timelines[id]`    | nothing; every value is a function of `useCurrentFrame()`           |
> | `data-start` / `data-duration` on a clip        | `<Sequence from>` / `durationInFrames`, in frames                  |
> | a CSS keyframe or transition                    | frame-derived inline `style` (CSS animation does not render)        |
>
> Never paste an example from this skill into a Remotion project unmodified.
...
For copyable combined picture/sound recipes, use `/hyperframes-core` → `references/editing-recipes.md`.
