...
// The directory holding the sibling skills (this skill lives one level inside it).
const BUCKET_DIR = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const HELPERS = [
  join(BUCKET_DIR, "hyperframes-animation", "scripts", "animation-map.mjs"),
  join(BUCKET_DIR, "hyperframes-creative", "scripts", "contrast-report.mjs"),
];
...
describe("package-loader parity", () => {
  it("package-loader.mjs is byte-identical to hyperframes-creative's copy (the stated contract)", () => {
    const here = readFileSync(
      join(BUCKET_DIR, "hyperframes-animation", "scripts", "package-loader.mjs"),
      "utf8",
    );
    const sibling = readFileSync(
      join(BUCKET_DIR, "hyperframes-creative", "scripts", "package-loader.mjs"),
      "utf8",
    );
...
// The WebGL/uniform shape, e.g. a shader template's uniform block
