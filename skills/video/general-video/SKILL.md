...
> ## ⚠ Contract status: not yet ported to Remotion
>
> This stack's output framework is **Remotion + React** (`/hyperframes-core`). This
> skill still describes the retired **HTML + `data-*` + paused-GSAP-timeline**
> pipeline, and its bundled scripts still emit `index.html` in that contract.
>
> Until it is ported:
>
> - **Do not** run `npx hyperframes <anything>` — that CLI is retired. `/hyperframes-cli`
>   maps every old command to its Remotion equivalent.
> - **Do not** copy its `data-*` markup or GSAP timeline code into a Remotion project.
> - **Do** take its craft from it: story shape, cut rhythm, visual design, the
>   decisions about what appears on screen. That part is framework-agnostic and is
>   why this skill is still here.
> - **Do** express the result through `/hyperframes-core`'s contract: `<Composition>`,
>   `<Sequence>`, and values derived from `useCurrentFrame()`.
>
> Say in your summary when you had to work around this, so the port can be finished.
...
Copy the full contracts from `../hyperframes-core/references/editing-recipes.md`.
...
| Any media asset or operation, including narration, BGM, SFX, captions, grading, or transforms                     | `/media-use`; for framework playback and placement also read `/hyperframes-core` → `references/media.md`                                                                                                                 |
